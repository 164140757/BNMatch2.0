...
/**
 * <p>Force function which computes an n-body prefuse.util.force such as gravity,
 * anti-gravity, or the results of electric charges. This function implements
 * the the Barnes-Hut algorithm for efficient n-body prefuse.util.force simulations,
 * using a quad-tree with aggregated mass values to compute the n-body
 * prefuse.util.force in O(N log N) time, where N is the number of ForceItems.</p>
 * 
 * <p>The algorithm used is that of J. Barnes and P. Hut, in their research
 * paper <i>A Hierarchical  O(n log n) prefuse.util.force calculation algorithm</i>, Nature,
...
public class NBodyForce extends AbstractForce {
	 ...
	 */
	public NBodyForce(boolean avoidOverlap) {
		this(DEFAULT_GRAV_CONSTANT, DEFAULT_DISTANCE, DEFAULT_THETA, avoidOverlap);
	}
	 ...
	 * n-body prefuse.util.force acting on a given item.
	...
	/**
	 * Calculates the prefuse.util.force vector acting on the given item.
	 * @param item the ForceItem for which to compute the prefuse.util.force
	 */
	...
	private void forceHelper(ForceItem item, QuadTreeNode n, float x1, float y1, float x2, float y2) {
		...
		if (avoidOverlap) {
			...
			if(isOverlapping) {
				effectivedx = 1 / effectivedx;
				effectivedy = 1 / effectivedy;
			}
			r = (float) Math.sqrt(effectivedx * effectivedx + effectivedy * effectivedy);
			if(r < 0.001f)
				r = 0.001f;
			if(Math.abs(effectivedx) < 0.00001f)
				effectivedx = 0.00001f;
			if(Math.abs(effectivedy) < 0.00001f)
				effectivedy = 0.00001f;
		}
		...
		if ((!n.hasChildren && n.value != item) || ((x2 - x1) / r < params[BARNES_HUT_THETA])) {
			if ( minDist ) return;

			// either only 1 particle or we meet criteria
			// for Barnes-Hut approximation, so calc prefuse.util.force
